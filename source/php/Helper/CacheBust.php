...
class CacheBust
{
    ...
    {
        $jsonPath = MEDIAUSAGE_PATH . apply_filters('MediaUsage/Helper/CacheBust/RevManifestPath', 'dist/rev-manifest.json');

        if (file_exists($jsonPath)) {
            return json_decode(file_get_contents($jsonPath), true);
        } elseif (WP_DEBUG) {
            echo '<div style="color:red">Error: Assets not built. Go to ' . MEDIAUSAGE_PATH . ' and run gulp. See '. MEDIAUSAGE_PATH . 'README.md for more info.</div>';
        }
    }
}
